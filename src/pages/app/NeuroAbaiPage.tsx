...
function ChatBubbleAvatar({ variant = "received" }: ChatBubbleAvatarProps) {
  return (
    <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${variant === "sent" ? "bg-[#1C7E66] text-white" : "bg-gray-200 text-[#1C7E66]"}`}>
...
function ChatBubbleMessage({ variant = "received", isLoading, children }: ChatBubbleMessageProps) {
  return (
    <div className={`rounded-lg p-3 max-w-[80%] ${variant === "sent" ? "bg-[#1C7E66] text-white" : "bg-gray-100 text-gray-900"}`}>
        ...
        <div className="h-[400px] overflow-y-auto p-4 bg-gray-50">
          {messages.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center text-center text-gray-400">
              <Bot size={48} className="mb-4 text-[#1C7E66]" />
              <h3 className="text-lg font-medium">Начните диалог с UIB College AI</h3>
        ...
        <div className="border-t p-4">
          <div className="relative rounded-lg border bg-white focus-within:ring-2 focus-within:ring-[#1C7E66]/20">
            ...
            <div className="flex items-center justify-between border-t p-2">
              ...
              <button onClick={handleSend} disabled={loading || (!input && files.length === 0)} className="ml-auto flex h-10 items-center gap-2 rounded-lg bg-[#1C7E66] px-4 text-white transition-colors hover:bg-[#1C7E66]/90 disabled:opacity-50">
                Отправить
                <Send size={16} />
              </button>
            </div>
