...
function ChatBubbleAvatar({ variant = "received" }: ChatBubbleAvatarProps) {
    return (
        <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${
            variant === "sent" ? "bg-[#1e40af] text-white" : "bg-gray-200 text-[#1e40af]"
        }`}>
...
function ChatBubbleMessage({ variant = "received", isLoading, children }: ChatBubbleMessageProps) {
    return (
        <div className={`rounded-lg p-3 max-w-[80%] ${
            variant === "sent" ? "bg-[#1e40af] text-white" : "bg-gray-100 text-gray-900"
        }`}>
            {isLoading ? (
        ...
        <div className="mt-4">
            ...
            <div
                ...
                className={`flex cursor-pointer flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed p-4 transition-colors ${
                    isDragging ? "border-[#1e40af]/50 bg-[#1e40af]/5" : "border-gray-300 bg-gray-50 hover:bg-gray-100"
...
export default function NeuroAbaiPage() {
    ...
    const handleSend = async () => {
        ...
        try {
            const response = await openaiResponsesApi.sendMessage({
                message: input,
                scenario,
                files
            });
    ...
    return (
        <div className="mx-auto max-w-3xl p-4">
            <div className="overflow-hidden rounded-xl border bg-white shadow-lg">
                {/* Заголовок и выбор сценария */}
                <div className="border-b p-4">
                    <h1 className="text-center text-xl font-bold text-[#1e40af]">FIZMAT.AI</h1>
                    <div className="mt-4">
                        <div className="relative">
                            <select
                                value={scenario}
                                onChange={e => setScenario(e.target.value)}
                                className="w-full appearance-none rounded-lg border bg-white px-3 py-2 pr-10 text-sm shadow-sm focus:border-[#1e40af] focus:outline-none focus:ring-2 focus:ring-[#1e40af]/20"
                            >
                                {SCENARIOS.map(s => (
                ...
                <div className="h-[400px] overflow-y-auto p-4 bg-gray-50">
                    {messages.length === 0 ? (
                        <div className="flex h-full flex-col items-center justify-center text-center text-gray-400">
                            <Bot size={48} className="mb-4 text-[#1e40af]" />
                            <h3 className="text-lg font-medium">Начните диалог с FIZMAT.AI</h3>
                ...
                <div className="border-t p-4">
                    <div className="relative rounded-lg border bg-white focus-within:ring-2 focus-within:ring-[#1e40af]/20">
                        <textarea
                            value={input}
                        ...
                        <div className="flex flex-col items-center justify-between border-t p-2">
                            ...
                            <button
                                ...
                                className="mr-auto flex h-10 items-center gap-2 rounded-lg bg-[#1e40af] px-4 text-white transition-colors hover:bg-[#1e40af]/90 disabled:opacity-50"
                            >
                                Отправить
                                <Send size={16} />
                            </button>
