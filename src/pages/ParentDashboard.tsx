...
import { FaUserFriends as FaUserFriendsRaw, FaCalendarAlt as FaCalendarAltRaw, FaComments as FaCommentsRaw, FaBell as FaBellRaw, FaUser as FaUserRaw, FaChartBar, FaBook as FaBookRaw } from 'react-icons/fa';

const FaUserFriends: any = FaUserFriendsRaw;
const FaCalendarAlt: any = FaCalendarAltRaw;
const FaComments: any = FaCommentsRaw;
const FaBell: any = FaBellRaw;
const FaUser: any = FaUserRaw;
const FaBook: any = FaBookRaw;

const ParentDashboard: React.FC = () => {
  // Мок-данные: казахские имена и процессы родителя
  const parent = { name: 'Гульнар Сериковна', child: 'Ерасыл Нурланулы', class: '8Б', email: 'gulnar@fizmat.kz' };
  ...
  const mockMessages = [
    { id: 1, from: 'Айгуль апай', text: 'Ваш сын хорошо сдал контрольную!' },
    { id: 2, from: 'Куратор', text: 'Родительское собрание завтра в 18:00.' },
  ];
  const mockNotifications = [
    { id: 1, text: 'Внимание: завтра родительское собрание.' },
    { id: 2, text: 'Добавлен новый отчёт об успеваемости.' },
  ];
  const mockStats = [
    { label: 'Успеваемость ребёнка', value: '91%' },
    { label: 'Сообщения', value: mockMessages.length },
    { label: 'Посещаемость', value: '100%' },
    { label: 'Класс', value: parent.class },
  ];
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
        ...
        <div className="flex flex-col items-start md:items-start">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-corporate-primary font-bold flex items-center gap-2 text-xl md:text-2xl">
              <FaUserFriends className="inline w-6 h-6 text-corporate-primary" />
              {parent.name}
            </span>
          </div>
          <span className="text-gray-500 text-sm">Ребёнок: {parent.child} ({parent.class})</span>
        </div>
        {/* Быстрые действия */}
            ...
            {showProfile && (
              <div className="absolute right-0 mt-2 w-72 max-w-xs sm:max-w-sm md:w-72 bg-white rounded-xl shadow-xl border animate-fadeIn z-30 overflow-hidden" style={{ width: '90vw', maxWidth: 300 }}>
                <div className="px-5 py-4 border-b bg-gray-50">
                  <div className="font-bold text-lg text-gray-800 mb-1">{parent.name}</div>
                  <div className="text-sm text-blue-600 mb-2">Родитель</div>
                </div>
                <div className="px-5 py-3 text-sm text-gray-700 border-b"><b>Email:</b> {parent.email}</div>
                <div className="px-5 py-3 text-sm text-gray-700"><b>Ребёнок:</b> {parent.child} ({parent.class})</div>
              </div>
            )}
          </div>
        </div>
      ...
      {/* Здесь могут быть другие виджеты для родителя: успеваемость, отчёты, активности и т.д. */}
    </div>
  );
};
